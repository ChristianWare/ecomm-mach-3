...
const CartItem = ({ item }: Props) => {
  ...
  const firstImageUrl = item?.images?.[0]?.asset?._ref
    ? urlFor({ _ref: item.images[0].asset._ref }).url()
    : "";
      ...
      <div className='flex col-span-5 md:col-span-2 items-center gap-4 ml-4'>
        <ImCross
          onClick={() => {
            ...
            toast.success(
              `${item?.name.substring(0, 12)}... deleted from cart`
            );
          }}
        ...
        <h1 className='font-semibold'>{item?.name.substring(0, 20)}</h1>
      </div>
